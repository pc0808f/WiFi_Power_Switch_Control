    ...
    try:
        ...
        if topic.decode() == (mq_topic + '/fota'):
            ...
            if data['commands'] == 'ping':
                ...
                if 'seconds' in data:
                    publish_MQTT_card_data(card_1, 'commandack-restart',data['state'],data['seconds'])
                else:
...
def publish_MQTT_card_data(card_data, MQTT_API_select, para1="", para2=""):  # 可以選擇card_1、claw_2、...，但MQTT_client暫時固定為mq_client_1
    ...
    if MQTT_API_select == 'commandack-getstatus' or  MQTT_API_select == 'status':
        ...
        macid = my_internet_data.mac_address
        mq_topic = '03ac6a6316d8fd2c7401249b6ca30a06/' + macid + '/' + token + '/commandack'
        if para1=="" :
            MQTT_card_data = {
                "ack": "pong",
                "time": utime.time()
        ...
        macid = my_internet_data.mac_address
        mq_topic = '03ac6a6316d8fd2c7401249b6ca30a06/' + macid + '/' + token + '/fotaack'
        if para1=="" :
            MQTT_card_data = {
                "ack": "OK",
                "time": utime.time()
        ...
        macid = my_internet_data.mac_address
        mq_topic = '03ac6a6316d8fd2c7401249b6ca30a06/' + macid + '/' + token + '/commandsack'
        gCommandOn=1
        if para1=="" :
            MQTT_card_data = {
                "ack": "on",
...
server_report_sales_period = 20 * 60  # 3分鐘 = 3*60 單位秒
# server_report_sales_period = 10   # For快速測試
server_report_sales_counter = -1 # -1代表第一次執行就要執行，0代表第一次執行不要執行
